...
class AddPromotionController extends AbstractController
{
     ...
     */
    public function addUser(ReferenceRepository $reporef,LangueRepository $repoLangue,FabriqueRepository $repoFabrique,SerializerInterface $serializer, Request $request, ValidatorInterface $validator, EntityManagerInterface $em, UserProfilRepository $repo, UserPasswordEncoderInterface $encoder)
    {
        ...
        foreach ($promotionTab['referentiels'] as $value) {
            ...
            if (!empty($value)) {
                $referentiel = $reporef->findBy(array('libelle' => $promotionTab['fabrique']))
            }
        }
        // Traitement Image
        $image = $request->files;
