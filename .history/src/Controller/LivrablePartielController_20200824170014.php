...
class LivrablePartielController extends AbstractController
{
     ...
     */
    public function showStatistiquesByApprenantId(int $id_apprenant, int $id_promo, int $id_ref, ApprenantRepository $repoApprenant, ReferentielRepository $repoReferentiel)
    {
        $apprenant = $repoApprenant->findOneBySomeField($id_apprenant,$id_promo);
        if (!$apprenant) {
            return new JsonResponse("Cet apprenant partiel n'existe pas dans cette promotion.", Response::HTTP_NOT_FOUND, [], true);
        }
