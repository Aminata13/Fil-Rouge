...
class AddPromotionController extends AbstractController
{
    ...
    {
        $serialiserExel = new Serializer([new ObjectNormalizer()], [new CsvEncoder()]);
        $fichier = $request->files->get('fichier')->getRealPath() ;
        dd(explode("/", $request->files->get('fichier')->getMimeType())[1]);
        $data = $serialiserExel->decode(file_get_contents($fichier),'(.xlsx)');
        dd($data);
