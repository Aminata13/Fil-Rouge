...
class AddPromotionController extends AbstractController
{
    ...
    */
    public function relanceAll(int $id_promo,PromotionRepository $repoPromo){
        $promo =  $repoPromo->find($id_promo);
        foreach ($promo->getApprenants() as $value) {
                if ($value->getAttente()) {
                    $value->getUser()->sendEmail(\Swift_Mailer $mailer,$value->getUser()->getPassword());
                }
        }
    }
     ...
     */
    public function relanceOne(int $id_promo,int $id_apprenant,PromotionRepository $repoPromo){
