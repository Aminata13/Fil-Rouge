...
class AddPromotionController extends AbstractController
{
     ...
     */
    public function addUser(FormateurRepository $repoFormateur,StatutRepository $repoStatus,ReferentielRepository $reporef,LangueRepository $repoLangue,FabriqueRepository $repoFabrique,SerializerInterface $serializer, Request $request, ValidatorInterface $validator, EntityManagerInterface $em, UserProfilRepository $repo, UserPasswordEncoderInterface $encoder)
    {
        ...
        foreach ($promotionTab['formateurs'] as $value) {
            if (!empty($value)){
                ...
                $promotion->addApprenant($formateurs); 
                $groupe->addApprenant($apprenant);
            }
        }
