...
class LivrablePartielController extends AbstractController
{
     ...
     */
    public function  putStatutByApprenant(Request $request, int $id_apprenant, int $id_livrable, StatutLivrableRepository $statutLivrable, ApprenantRepository $repoApprenant, LivrablePartielRepository $repoLivrablePartiels)
    {
        $data = json_decode($request->getContent(), true);
        $livrablePartiel = $repoLivrablePartiels->find($id_livrable);
        $apprenant = $repoApprenant->find($id_apprenant);
        $statut = statutLivrable->find
