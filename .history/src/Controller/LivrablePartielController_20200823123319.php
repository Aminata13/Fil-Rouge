...
class LivrablePartielController extends AbstractController
{
     ...
     */
    public function  putStatutByApprenant(int $id_apprenant, int $id_livrable, ApprenantRepository $repoApprenant, LivrablePartielRepository $repoLivrablePartiels)
    {
        $livrablePartiel = $repoLivrablePartiels->find($id_livrable);
        $apprenant = $repoApprenant->find($id_apprenant);
        if ($apprenant != $livrablePartiel->getLivrableRendus()->getApprenants()) {
            # code...
        }
    }
}
