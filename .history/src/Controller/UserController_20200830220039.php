...
class UserController extends AbstractController
{
    ...
    public function updateInfoapprenant(UserPasswordEncoderInterface $encoder, EntityManagerInterface $em, SerializerInterface $serializer,Request $request,ApprenantRepository $repoApre)
    {
        ...
        $aprenant = $repoApre->findOneByIdUser($user->getId());
        if (!$aprenant){
            $re = $serializer->serialize(["msg"=>"l'apprenant existe pas"], 'json');
            return new JsonResponse($re, Response::HTTP_OK, [], true);
        }
        $data = json_decode($request->getContent(),true);
        if (count($data)<1) {
            $re = $serializer->serialize(["response"=>"les donnes sont aubligatoire"],'json');
            return new JsonResponse($re, Response::HTTP_OK, [], true);
        }
        ...
        $em->flush();
        $re = $serializer->serialize(["response"=>"succes"],'json');
        return new JsonResponse($re, Response::HTTP_OK, [], true);
    }
}
