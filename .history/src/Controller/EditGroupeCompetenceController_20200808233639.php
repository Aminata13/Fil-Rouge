...
class EditGroupeCompetenceController extends AbstractController
{
     ...
     * )
     */
    public function editGroupeCompetence(int $id ,GroupeCompetenceRepository $repo, SerializerInterface $serializer, EntityManagerInterface $em, ValidatorInterface $validator,Request $request)
    {
        

        $data=json_decode($request->getContent(),true);
        dd($data);
        $groupeCompetence = $repo->find($id);
        $groupeCompetence->setLibelle($data['libelle']);
        $groupeCompetence->setLibelle($data['description']);
