...
class UserController extends AbstractController
{
     ...
     */
    public function updateInfoapprenant(EntityManagerInterface $emSerializerInterface $serializer,Request $request,ApprenantRepository $repoApre)
    {
        $user = $this->getUser();
        $aprenant = $repoApre->findOneByIdUser($user->getId());
        if (!$aprenant){
        ...
        // $em->persist($aprenant);
        $em->flush();
        $userJson = $serializer->serialize($data['nom'],'json');
        return new JsonResponse($userJson, Response::HTTP_OK, [], true);
    }
}
