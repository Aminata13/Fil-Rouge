        ...
        foreach ($referentiel->getGroupeCompetences() as $groupeCompetence) {
            foreach ($groupeCompetence->getCompetences() as $competence) {
                ...
                foreach ($promo->getApprenants() as $apprenant) {
                    $competenceValide = $repoComptenceValide->findOneBySomeField($id_promo, $id_ref, $competence->getId(), $apprenant->getId());
                    if ($competenceValide) {
                        if ($competenceValide->getNiveau1()) {
                            $niveau1 = $niveau1 +1;
                        }else if ($competenceValide->getNiveau2()) {
                            $niveau2 = $niveau2 +1;
                        }else if ($competenceValide->getNiveau3()) {
                            $niveau3 = $niveau3 +1;
                        }
                    }
                }
                
                $statistiqueCompetences[] = ["id"=>$competence->getId(),"libCompetence"=>$competence->getLibelle(),"niveau1"=>$niveau1,"niveau2"=>$niveau2,"niveau3"=>$niveau3];
                
            }
        }
        $data = $serializer->serialize($statistiqueCompetences, "json");
        return new JsonResponse($data, Response::HTTP_OK, [], true);
    }
