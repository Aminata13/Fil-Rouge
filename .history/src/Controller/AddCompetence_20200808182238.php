...
class AddCompetence
{
    ...
    public function __invoke(Competence $data)
    {
        ...
        $grpComp =$this->repo->findBy(array('libelle' => $groupeCompetence))[0];
        if (!$grpComp) {
            return new JsonResponse("Ce groupe de competence n'existe .", Response::HTTP_BAD_REQUEST, [], true);
        }
        $competence->addGroupeCompetence($grpComp);
