...
class LivrablePartielController extends AbstractController
{
     ...
     */
    public function  putLivrablePartielByFormateur(\Swift_Mailer $mailer, SerializerInterface $serializer, int $id_promo, int $id_brief, Request $request, EntityManagerInterface $em, StatutLivrableRepository $repoStatut, ApprenantRepository $repoApprenant, NiveauEvaluationRepository $repoNiveau, PromotionRepository $repoPromo, BriefRepository $repoBrief, LivrablePartielRepository $repoLivrablePartiels)
    {
            ...
            if (isset($data['deleted']) && $data['deleted']) {
                ...
                foreach ($briefPromotions as $value) {
                    if ($value->getBrief() == $brief) {
                        $value->removeLivrablePartiel($livrablePartiel);
                    } 
                }
                $em->flush();
        ...
        $livrablePartiel->setDateAffectation(new \D);
