...
class LivrablePartielController extends AbstractController
{
    ...
    public function showStatistiquesByCompetences(SerializerInterface $serializer, int $id_promo, int $id_ref,ReferentielRepository $repoReferentiel, PromotionRepository $repoPromo, CompetenceValideRepository $repoComptenceValide)
    {
        ...
        foreach ($referentiel->getGroupeCompetences() as $groupeCompetence) {
            foreach ($groupeCompetence->getCompetences as $key => $value) {
                # code...
            }
            foreach ($promo->getApprenants() as $apprenant) {
                $competenceValide = $repoComptenceValide->findOneBySomeField($id_promo,$id_ref,$groupeCompetence->getCompetenes(),$apprenant->getId());
            }
        }
        //
        //dd($competenceValide);
