...
class LivrablePartielController extends AbstractController
{
     ...
     */
    public function  putLivrablePartielByFormateur(int $id_promo, int $id_brief, Request $request, EntityManagerInterface $em, PromotionRepository $repoPromo, BriefRepository $repoBrief, LivrablePartielRepository $repoLivrablePartiels)
    {
        ...
        if (empty($promo) || !$promo->getGroupes()->contains($brief)) {
        ...
        if (empty($data['dateAffectation'])) {
            return new JsonResponse('La date d\'affectation est requis.', Response::HTTP_BAD_REQUEST, [], true);
        }

        if (empty($data['dateSoumission'])) {
            return new JsonResponse('La date de soumission est requis.', Response::HTTP_BAD_REQUEST, [], true);
        }
